...
def test_flex_csv_parses_deposits_and_dividends():
    csv_payload = """Date,Symbol,Description,Amount,CurrencyPrimary,Quantity,Price,Commission
2025-01-15,CASH,Electronic Fund Transfer Deposit,25000,USD,1,25000,0
2025-03-01,MSFT,Dividend Payment,37.35,USD,45,0.83,0
"""
    rows = _parse_flex_csv("U123", csv_payload)
    assert len(rows) == 2
    assert rows[0].action == "deposit"
    assert rows[1].action == "dividend"


def test_flex_action_mapping():
    assert _map_flex_action("Wire Deposit", 1000) == "deposit"
    assert _map_flex_action("Withdrawal to bank", -500) == "withdrawal"
    assert _map_flex_action("Stock Split", 0) == "corporate_action"
...
def test_benchmark_alignment_uses_portfolio_dates():
    ...
    comparison = align_benchmark_comparison(history, allow_mock=True)
    assert comparison.get("aligned_observations") == 2
    assert "portfolio_return_percent" in comparison
...
def test_point_in_time_fundamentals_store(tmp_path, monkeypatch):
    ...
    seed_walk_forward_demo_records("MSFT", base)
    snapshot = get_point_in_time_fundamentals("MSFT", date.today())
